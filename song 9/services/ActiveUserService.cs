...
public class ActiveUserService : IActiveUser
{
    ...
    public ActiveUserService(IHttpContextAccessor context)
    {
        ...
        var userId = userClaims?.FindFirst("userID") ?? userClaims?.FindFirst("Id") ?? userClaims?.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier);
        var userName = userClaims?.FindFirst("username") ?? userClaims?.FindFirst(System.Security.Claims.ClaimTypes.Name);
        if (userId != null)
        {
            ActiveUser = new User
            {
